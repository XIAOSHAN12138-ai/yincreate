...
> **注意**:本文里出现的两个 ID 不能混用:
> - `media_id` —— 我方素材库业务主键(形如 `MEDIA-D76BB07D`),由前端传给后端
> - `seedance_resource_uuid` —— Neolink 资源库 UUID(形如 `tkres_abc123def456`),由后端在素材库里持久化,前端不需要接触
> - `Asset://tkres_xxx` —— Seedance 上游识别的引用协议,由后端在请求时构造
...
| 字段 | 类型 | 是否必填 | 说明 |
|---|---|---|---|
| `media_id` | string | **推荐必填**(人脸场景) | 我方素材库业务主键(形如 `MEDIA-D76BB07D`),后端用它查 `media_library` 行,自动完成 Neolink 资源库注册与 `Asset://tkres_xxx` 改写 |
