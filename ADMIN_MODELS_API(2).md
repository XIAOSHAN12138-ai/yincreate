...
> [`50_ai_model_upstream_routes.sql`](../../database/sql/schema/50_ai_model_upstream_routes.sql)（**PR-4.13 路由表**）

> ## 📌 PR-4.13（2026-08-19）：上游路由拆分到独立表 `ai_model_upstream_routes`
>
> 之前 `ai_models` 上有两个字段都是"按分辨率路由上游 ID"，却分散在主表 JSONB 与 `api_params` 嵌套键：
> - `upstream_id_by_resolution` JSONB `dict[str, str]`（PR-4.6，vendor 路由热路径唯一消费源）
> - `api_params.resolution_variants` JSONB `dict[str, list[str]]`（PR-4.10，仅 admin UI 写、vendor 路由从不读）
>
> 用户原话："分辨率变体和上游模型id都是基于分辨率的，但他们之间没有任何关系，所以将他们写在一起是完全不合理的。"
>
> PR-4.13 把这两块数据合并迁到独立表 `ai_model_upstream_routes`，结构：
> ```
>   model_id | resolution_key | upstream_id | priority_order
>   (1 行 = 一个 (model, resolution) 候选; priority_order=1 首选, ≥2 回退)
> ```
>
> **接口形态零变更**：admin API 仍接受 `upstream_id_by_resolution: dict[str, str]` 与 `resolution_variants: dict[str, list[str]]`，响应也仍以同样形态返回。底层改成读/写新表。
>
> **存储位置变更**：
> - ❌ 不再写 `ai_models.upstream_id_by_resolution` 列（阶段 4 DROP）
> - ❌ 不再写 `ai_models.api_params.resolution_variants` 嵌套键（阶段 4 清掉）
> - ✅ 写到 `ai_model_upstream_routes` 表
>
> **路由优先级语义**：`resolution_variants[res]=[id_a, id_b]` 现在展开成 `(res, priority=2, id_a)` + `(res, priority=3, id_b)`，与 `upstream_id_by_resolution[res]` 的 `(res, priority=1, id)` 形成完整优先级链。当前 vendor 路由实现仍只取 1，但表结构已为后续"按序回退"实现预留。
>
> **回退阶段**：阶段 1 (CREATE TABLE) → 阶段 2 (回填) → 阶段 3 (代码读新表 + JSONB fallback) → 阶段 4 (DROP 旧列 + 清嵌套键，需 1-2 周监控稳定后)。清理 SQL: [`51_drop_ai_models_upstream_routes_cleanup.sql`](../../database/sql/schema/51_drop_ai_models_upstream_routes_cleanup.sql)
...
>
> - **存储位置**：阶段 4 前持久化到 `ai_models.api_params` JSONB 的 `resolution_variants` 键下；PR-4.13 后写到 `ai_model_upstream_routes` 表（priority_order 列承载回退顺序）
...
| **`upstream_id_by_resolution`** | **Dict[str, str]** | **NOT NULL DEFAULT `{}`** | **🆕 PR-4.6 必填 / PR-4.13 存储位置变更** — 上游私有 ID 路由表（首选映射，分辨率键 → 上游 ID 值）。多分辨率 happyhorse：`{"720P": "happyhorse-1.0-i2v-720p", "1080P": "happyhorse-1.0-i2v-1080p"}`；单上游 gpt-image-2：`{"all": "gpt-image-2"}`。**PR-4.13 起，DTO 字段名/形态不变，但底层持久化到 `ai_model_upstream_routes` 表的 priority_order=1 行**（`ai_models.upstream_id_by_resolution` JSONB 列将在阶段 4 被 DROP，详见 §2.6）。想要回退顺序请配合 `resolution_variants` 一起传 |
...
### 2.5 `resolution_variants` 分辨率变体映射（PR-4.10，PR-4.13 重写）

> **PR-4.13 起**：本字段的存储位置从 `ai_models.api_params` JSONB 嵌套键迁移到独立表 `ai_model_upstream_routes`（详见 §2.6）。**DTO 字段名/形态仍为 `dict[str, list[str]]`**，前端协议零变更。
...
- 响应体顶层独立字段：`_row_to_response_dict` 仍会把 `resolution_variants` 提到顶层
- 存储位置（**PR-4.13**）：
  - 阶段 1-3：写到独立表 [`ai_model_upstream_routes`](../../database/sql/schema/50_ai_model_upstream_routes.sql)，每条 `(resolution_key, upstream_id)` 展开为一行，`priority_order = 2 + index`。同时**也**会从 `ai_models.api_params.resolution_variants` JSONB 嵌套键读回（fallback 兼容）
  - 阶段 4：`api_params.resolution_variants` 嵌套键被 `UPDATE ai_models SET api_params = api_params - 'resolution_variants'` 清掉，见 [`51_drop_ai_models_upstream_routes_cleanup.sql`](../../database/sql/schema/51_drop_ai_models_upstream_routes_cleanup.sql)

**字段语义（统一）**：

| 请求值 | 含义 | 副作用（PR-4.13 写新表） |
| --- | --- | --- |
| `null`（字段未提供或显式 null） | 不动现有值 | 无 |
| `{}` | 清空 | `DELETE FROM ai_model_upstream_routes WHERE model_id = ?` |
| `{"720P": ["a", "b"]}` | 设置该映射 | INSERT `(720P, a, priority=2)` + `(720P, b, priority=3)`（顺序保留） |

**校验规则**（DAO 内部静默丢弃非法项，不报错）：

- 键必须为非空字符串（resolution_key VARCHAR(64)）
- 值必须为 list；list 元素全部转 str；None 元素丢弃
- 非 dict / 非 list 的整体值 → 当作空处理

**与 `upstream_id_by_resolution` 的合并规则**（admin handler 内 `_build_routes_payload` 实现）：

- `upstream_id_by_resolution["720P"] = "primary-id"` → 新表 `(720P, primary-id, priority=1)`
- `resolution_variants["720P"] = ["alt-1", "alt-2"]` → 新表 `(720P, alt-1, priority=2)` + `(720P, alt-2, priority=3)`
- 两个字段都给同一个分辨率时：`upstream_id_by_resolution` 是首选（priority=1），`resolution_variants` 是回退（priority≥2）
- 按 `(resolution_key, priority_order)` UNIQUE，**重复 ID 不会写重复行**（同样的 ID 在两个字段都给出也只会占一个 priority 槽位）

**示例：给 happyhorse_1_0 设置 720P 双候选 + 1080P 单上游**
...
| model_id | resolution_key | upstream_id | priority_order |
| --- | --- | --- | --- |
| `happyhorse_1_0` | `720P` | `happyhorse-1.0-i2v-720p` | 1 |
| `happyhorse_1_0` | `720P` | `happyhorse-alt-720p` | 2 |
| `happyhorse_1_0` | `1080P` | `happyhorse-1.0-i2v-1080p` | 1 |

**示例：清空**（只清 `resolution_variants`，不动 `upstream_id_by_resolution`）：
...
→ DB 写入：DELETE 所有 priority ≥ 2 的行，priority=1 保留。
...
**响应示例**（GET 详情 / PATCH 后返回，wire format 不变）：
...
> **注意**：PR-4.13 起，`api_params.resolution_variants` 子键不再被 admin API 自动同步。前端请从顶层 `resolution_variants` 字段读取（保留 PR-4.10 兼容形态）。

**Clone 行为**（`POST /api/v1/admin/models/{model_id}/clone`）：

- 源模型的 `ai_model_upstream_routes` 全表自动复制到新模型（同 model_id FK 行全部 clone，priority_order 保持）
- `CloneModelRequest.resolution_variants` 可覆盖（与 `upstream_id_by_resolution` 一并传给 `_build_routes_payload`）；不传 = 用源值；`{}` = 仅清空变体；`{...}` = 设置

**Changelog diff**：当 PATCH 改 `upstream_id_by_resolution` 或 `resolution_variants`，会在 `ai_model_changelog.changed_fields` 写 `{"upstream_id_by_resolution": {"old": {...}, "new": {...}}}` 或 `{"resolution_variants": {"old": {...}, "new": {...}}}`（PR-4.10 起，PR-4.13 路径不变）。

### 2.6 `ai_model_upstream_routes` 路由表（PR-4.13）

> **新表** [`ai_model_upstream_routes`](../../database/sql/schema/50_ai_model_upstream_routes.sql) — vendor 路由上游 ID 的单一可信源（single source of truth）。

**动机**：PR-4.13 之前，同一份"按分辨率路由上游 ID"数据分散在两个地方：

| 字段 | 来源 PR | 类型 | 用途 | 状态 |
| --- | --- | --- | --- | --- |
| `ai_models.upstream_id_by_resolution` | PR-4.6 | JSONB `dict[str, str]` | vendor 路由热路径唯一消费源 | ❌ 阶段 4 DROP |
| `ai_models.api_params.resolution_variants` | PR-4.10 | JSONB `dict[str, list[str]]` | 仅 admin UI 写，vendor 路由从不读 | ❌ 阶段 4 清掉 |

用户原话："分辨率变体和上游模型id都是基于分辨率的，但他们之间没有任何关系，所以将他们写在一起是完全不合理的。"

→ PR-4.13 把两者合并迁到 `ai_model_upstream_routes` 一张表，按行表达回退顺序。

**Schema**：

```sql
CREATE TABLE ai_model_upstream_routes (
    ...
    UNIQUE (model_id, resolution_key, priority_order),
    FOREIGN KEY (model_id) REFERENCES ai_models(model_id) ON DELETE CASCADE
);
CREATE INDEX idx_ai_model_upstream_routes_lookup
    ON ai_model_upstream_routes(model_id, resolution_key, priority_order);
```
...
| 列 | 类型 | 语义 |
| --- | --- | --- |
| `model_id` | VARCHAR(100) FK | 业务主键（FK → `ai_models.model_id`，`ON DELETE CASCADE`） |
| `resolution_key` | VARCHAR(64) | 视频：`720P` / `1080P`；图像：`1024x1024`；特殊值：`all`（单上游兜底） |
| `upstream_id` | VARCHAR(100) | 上游私有 ID（`vendor_b` 直接传） |
| `priority_order` | INT | 优先级，`1`=首选，`2`=第一次回退，`3`=第二次回退 …… |

**UNIQUE 约束**：`(model_id, resolution_key, priority_order)` — 同一对 `(模型, 分辨率)` 的同一优先级只能有 1 行，保证回填幂等。

**FK 行为**：model 软删时路由行保留（`deleted_at IS NOT NULL`）；model 硬删时路由行 `ON DELETE CASCADE` 一并删除。

**数据流**：

```
admin POST/PATCH 请求 (resolution_variants / upstream_id_by_resolution)
    ↓ admin handler: _build_routes_payload(payload)
    ↓   → priority=1: payload['upstream_id_by_resolution']
    ↓   → priority≥2: payload['resolution_variants'] 按 list 顺序
    ↓ DAO: set_upstream_routes(model_id, {res: [id1, id2, ...]})
    ↓   → DELETE old rows for model_id
    ↓   → INSERT new rows (priority = 1, 2, 3, ...)
    ↓ ai_model_upstream_routes 表
    ↓ vendor 路由热路径: token_switch_helper.get_upstream_id_for_resolution()
    ↓   → SELECT priority=1 WHERE (model_id, resolution_key)
    ↓   → fallback: ('all' key)
    ↓   → fallback: ai_models.upstream_id_by_resolution JSONB (阶段 3)
    ↓   → fallback: ai_models.upstream_model_id 列
```
...
| 阶段 | 内容 | 风险 |
| --- | --- | --- |
| 阶段 1 | 跑 `50_ai_model_upstream_routes.sql` 的 §1-§4（CREATE TABLE + 索引 + 触发器 + 注释）。新表为空，旧字段完全不动 | 零 |
| 阶段 2 | 同一 SQL 文件的 §5-§6（数据回填）。新表有数据，旧字段继续为 vendor 路由服务 | 零（回填 ON CONFLICT DO NOTHING 幂等） |
| 阶段 3 | 代码改动：DAO `get/set_resolution_variants` 改读/写新表，`token_switch_helper.get_upstream_id_for_resolution()` 优先查新表，旧字段成 fallback | 中（缓存陈旧 30s TTL 自愈） |
| 阶段 4 | 跑 `51_drop_ai_models_upstream_routes_cleanup.sql`：`DROP COLUMN upstream_id_by_resolution` + `UPDATE api_params = api_params - 'resolution_variants'`。代码移除 JSONB fallback | 不可逆（需先 DB 备份 + 2 周阶段 3 监控） |

**vendor 路由当前实现（阶段 3 行为）**：

`app/utils/token_switch_helper.py:get_upstream_id_for_resolution()` 的查询顺序：

1. `ai_model_upstream_routes` 表 `WHERE (model_id, resolution_key, priority_order=1)` — 命中即返回
2. `ai_models.upstream_id_by_resolution` JSONB 字段（PR-4.6 fallback，仅阶段 3 有效）
3. `ai_models.upstream_model_id` 列兜底（单上游场景）

> **未来增强**：表结构已支持 priority≥2，但当前实现只取 priority=1。后续 PR 可改为：取 priority=1 失败时按 priority_order 递增尝试下一个 `_upstream_id`，实现真正的"按序回退"。

**DAO 接口**（`database/dao/ai_model_dao.py`）：

```python
# 读所有路由行, 按 (resolution_key, priority_order) 排序
def get_upstream_routes(model_id: str) -> List[Tuple[str, int, str]]:
    """返回 [(resolution_key, priority_order, upstream_id), ...]"""

# 内部工具: rows → {res: [id1, id2, ...]} (按 priority_order 升序)
def routes_to_dict(rows: List[Tuple[str, int, str]]) -> Dict[str, List[str]]:

# 原子写: None=noop, {}=清空, 非空 dict=DELETE+INSERT
def set_upstream_routes(model_id: str, routes: Optional[Dict[str, List[str]]]) -> None:

# 兼容 shim (PR-4.13 阶段 3 过渡): 从 payload 弹 resolution_variants, 不再写 api_params
def merge_resolution_variants_into_payload(payload: Dict) -> Dict:
    """只 pop('resolution_variants', None), 不再合并到 api_params"""
```

**Admin handler 入口**（`app/api/admin/models.py`）：

```python
def _build_routes_payload(payload: Dict[str, Any]) -> Optional[Dict[str, List[str]]]:
    """
    把请求里的 upstream_id_by_resolution + resolution_variants 合并成
    单一 routes dict (priority 链), 返回:
      None = 请求里两个字段都没给, 不动路由
      {}   = 请求显式清空 (priority=1 也清掉)
      {..} = 归一化后的 routes 字典
    """
```
...
| **`upstream_id_by_resolution`** | **✅** | **🆕 PR-4.6** — 路由表首选映射 (priority=1) PATCH; **PR-4.13 改走** `_build_routes_payload` → `ai_model_upstream_routes` 表 (不再直接 UPDATE `ai_models.upstream_id_by_resolution` 列, 阶段 4 后该列不存在); 传 `{}` 或 `null` 表示不动现有表 |
| **`resolution_variants`** | **✅** | **🆕 PR-4.10 / PR-4.13 改走新表** — 路由表回退映射 (priority≥2) PATCH; 同上经 `_build_routes_payload` → `ai_model_upstream_routes`; 传 `{}` 清空所有 priority≥2 行, priority=1 保留 |
