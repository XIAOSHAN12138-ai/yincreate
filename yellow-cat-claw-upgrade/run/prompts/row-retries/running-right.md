Create Codex pet row `running-right` for `yellow-cat-claw`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Upgrade existing v1 atlas to v2 with 16 clockwise look directions; preserve the cropped yellow cat claw identity, dark brown overhead slab, teal base accent, warm photographic texture, and transparent sprite extraction.. Preserve silhouette, face, palette, material, proportions, markings, and props.
